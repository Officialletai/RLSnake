...
class DQNAgent:
    def __init__(self, input_size=12, output_size=4, lr=0.0025, gamma=0.995, max_epochs=10000, epsilon_start=1.0, epsilon_min=0.0001, epsilon_decay_rate=0.9975, display_game=True):
        ...
        self.target_update = 10  # The frequency with which the target network is updated
        self.target_model.load_state_dict(self.model.state_dict())  # Initialize target_model weights with the model weights
    ...
    def train(self, state, action, reward, next_state, done, steps):
        ...
        self.epsilon = max(((self.epsilon_decay_rate ** (steps+1))), self.epsilon_min)
    ...
    def dqn_learn(self):
        for i in range(self.max_epochs):
            ...
            while not done:
                ...
                self.train(state, action, reward, next_state, done, i)
                state = next_state
